...
# Feat: The Interface (CLI)
We'll use a library called rich to make the terminal output look like a proper app (colored, loading spinners, panels).
...
- argparse: This built-in library lets us pass flags like --vault and --query from the command line.
- rich: Notice how console.status creates a context manager (with ...)? This handles the spinner animation for us automatically while the code inside the block runs.
